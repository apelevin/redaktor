...
export default function SkeletonChatPanel({ selectedItems, onAllItemsProcessed }: SkeletonChatPanelProps) {
  ...
  useEffect(() => {
    if (selectedItems.length > 0 && currentItemIndex < selectedItems.length && !isLoading && !currentQuestion && !allItemsProcessed) {
      ...
      // Проверяем, что пункт еще не обработан и не обрабатывается
      if (!processedItems.has(itemKey) && processingItemKey !== itemKey) {
        processNextItem();
      }
    }
  }, [currentItemIndex, selectedItems.length, isLoading, currentQuestion, allItemsProcessed, processedItems, processingItemKey]);
  ...
  const processNextItem = async () => {
    ...
    try {
      ...
      if (data.question) {
        ...
        // Используем функциональное обновление messages, чтобы получить актуальное состояние
        setMessages((prevMessages) => {
          ...
          const questionAlreadyInMessages = prevMessages.some(
            msg => {
          ...
          if (!questionAlreadyInMessages) {
            // Возвращаем обновленный массив с новым вопросом
            return [
              ...prevMessages,
              {
                id: Date.now().toString(),
          ...
          // Вопрос уже есть, возвращаем без изменений
          return prevMessages;
        });
