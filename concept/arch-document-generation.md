...
- **Скелета документа** (структура с разделами и пунктами), созданного на шаге 3
...
3. Формирует итоговый документ с заголовками разделов
...
3. **Загрузка и рендеринг промпта:**
...
4. **Вызов OpenAI API:**
   ...
   - Поддерживает `reasoning_effort` и `verbosity` (если доступны)
...
2. **Генерация пункта:**
   ...
   ↓
   Форматирование данных → Загрузка промпта → OpenAI API
   ↓
...
3. **Сборка документа:**
   ...
   └─ Формирование структуры с заголовками
   ↓
