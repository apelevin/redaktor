...
interface Question {
  ...
  isRequired: boolean;       // обязателен ли вопрос
...
4. **Используй conditionalText для вопросов "да/нет" с детализацией**:
...
5. **Определяй dependsOn** - если вопрос зависит от других полей, укажи их пути в `dependsOn`.

6. **Определяй affects** - укажи пути в контексте, на которые влияет ответ на этот вопрос.

7. **Если контекст достаточен** - верни `null` вместо объекта Question.
...
### Пример 3: Вопрос с условным текстовым полем (да/нет + детали)
```json
{
